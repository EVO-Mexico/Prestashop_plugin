...
class EVOMexicoResponseModuleFrontController extends ModuleFrontController
{
    private $evomexico;
   ...
   public function initContent()
    {
        ...
        if(Tools::getValue('status') ==='cancel'){ //cancel only
            ...
            $statusCheckRes = $this->evomexico->getPaymentStatus($merchantTxId);
        ...
        // load order
        $raw = $this->evomexico->queryEvoPaymentsByMerchantTxId($merchantTxId);
        ...
        $this->evomexico->updateEvoPaymentsStatus($merchantTxId, $finalStatus);
        $this->evomexico->updateIdOrderInEvoPayment($raw['id_evo_payment'], $raw['id_order_raw']);
    ...
    private function payByEvo($id_evo_payment, $status, $order, $retry = 0)
    {
        ...
        if((int)$retry!==0){
            $this->evomexico->updateOrderStatus($id_order_state, $order);
        }
