...
include_once _PS_MODULE_DIR_ . '/evopayments/translations/statuses.php';

class EVOPayments extends PaymentModule
{
    ...
    const ST_EVO_CASHIER_URL_SANDBOX = "https://cashierui-apiuat.test.evomexico.mx/ui/cashier";
    const ST_EVO_JAVASCRIPT_URL_SANDBOX = "https://cashierui-apiuat.test.evomexico.mx/js/api.js";
    const ST_EVO_TOKEN_URL_SANDBOX = "https://apiuat.test.evomexico.mx/token";
    const ST_EVO_PAYMENT_URL_SANDBOX = "https://apiuat.test.evomexico.mx/payments";
   
    const ST_EVO_CASHIER_URL_LIVE = "https://cashierui-api.evomexico.mx/ui/cashier";
    const ST_EVO_JAVASCRIPT_URL_LIVE = "https://cashierui-api.evomexico.mx/js/api.js";
    const ST_EVO_TOKEN_URL_LIVE = "https://api.evomexico.mx/token";
    const ST_EVO_PAYMENT_URL_LIVE = "https://cashierui-api.evomexico.mx/ui/cashier";
    //DEFAULT TO IFRAME
    // 1 - IFRAME
    // 0 - REDIRECT
    // 2 - HOSTED PAY
    ...
    {
        $this->name = 'evopayments';
        $this->displayName = 'EVOPayments';
        $this->tab = 'payments_gateways';
        $this->version = '1.1.0';
        $this->author = 'EVOPayments';
        ...
        $this->displayName = $this->l('EVOPayments');
        $this->description = $this->l('Accepts payments by EVOPayments');
    ...
    {
        Payments\Config::$MerchantId = Configuration::get('EVO_MERCHANT_ID');
        Payments\Config::$Password = Configuration::get('EVO_PASSWORD');
        if (Configuration::get('EVO_SANDBOX') == 1) {
            Payments\Config::buildEvoEnv4Test(
                Configuration::get('EVO_CASHIER_URL_SANDBOX'),
                Configuration::get('EVO_JAVASCRIPT_URL_SANDBOX'),
                Configuration::get('EVO_TOKEN_URL_SANDBOX'),
                Configuration::get('EVO_PAYMENT_URL_SANDBOX')
                );
        } else {
            Payments\Config::buildEvoEnv4Prod(
                Configuration::get('EVO_CASHIER_URL_LIVE'),
                Configuration::get('EVO_JAVASCRIPT_URL_LIVE'),
                Configuration::get('EVO_TOKEN_URL_LIVE'),
                Configuration::get('EVO_PAYMENT_URL_LIVE')
                );
        }
    }
    ...
    public function displayForm()
    {
        $form['evopayments'] = [
            ...
            $form['merchant'] = [
                'form' => [
                    'legend' => [
                        'title' => $this->l('EVOPayments Connection settings'),
                        'icon' => 'icon-cog'
                    ],
                    'input' => [
                        [
        ...
        $this->addIntegrationMode($form['evopayments']['form']['input']);
    ...
    public function buildTemplatePath($name)
    {
        ...
        return 'module:evopayments/views/templates/front/' . $name . '17.tpl';
    }
    ...
    public function hookPaymentOptions($params)
    {
        ...
        $paymentOption->setCallToActionText($this->l('Pay with EVOPayments'))
...
//         $langCode = (new Language($order->id_lang)) -> iso_code;
        ...
        if (Configuration::get('EVO_REFUND_POSSIBILITY') && $order->module == 'evopayments') {
            $lastStatus = 0;
            ...
            if ($orders) {
                foreach ($orders as &$orderr) {
                    ...
                    if($rawInfo !== ""){
                        ...
                        $orderr['info'] = $this->l('Refunded: ', 'evopayments').$rawInfo;
                    }
    ...
    {
        $refundAmount = Tools::getValue('refundValue');
        $sum = $this->getSumCurrentOrderPayments($order);
        $possibleToRefund = $order->total_paid + $sum;
        
        $refundAmount = (double)$refundAmount;
        
        if($possibleToRefund < 0 || round($refundAmount,2) > round($possibleToRefund,2)){
            return ['state' => 'danger', 'text' => $this->l('You can not make a refund on this amount for this order.', 'evopayments')];
        }
        ...
        if(!$refund->errors){
            ...
            $id_evo_payment = $this->addOrderPaymentToDB($order->id_cart, $refundTxId, $status='refunded', '-'.$refundAmount, $order->id, $refundAmount.$currency->iso_code);
            //$order->addOrderPayment('-'.$refundAmount, 'EVOPayments', $id_evo_payment, $currency, date('Y-m-d H:i:s'));
            ...
            return ['state' => 'danger', 'text' => $this->l('No connect with payment gateway.', 'evopayments')];
        }
        return false;
    }
    ...
    private function getSumCurrentOrderPayments($order)
    {
        ...
        foreach ($payments as $payment) {
            if($payment->payment_method=='EVOPayments'){
    ...
    {
        $link = $this->context->link->getModuleLink('evopayments', 'payment');
    ...
    {
        $paymentOptions = [
            'cta_text' => $this->l('Pay with EVOPayments'),
            'action' => $this->context->link->getModuleLink('evopayments', 'payment'),
    ...
    public function getPaymentToken($merchantTxId, $retry='', $mode, $amount='')
    {
        ...
        try {
           ...
           $notificationUrl = Tools::getHttpHost(true).__PS_BASE_URI__ . 'index.php?fc=module&module=evopayments&controller=response&ipg=0&retry=' . $retry;
//             debug locally
//              $notificationUrl = 'http://externalip:8866/presta175/index.php?fc=module&module=evopayments&controller=response&ipg=1&retry=' . $retry;
            $landingUrl = Tools::getHttpHost(true).__PS_BASE_URI__ . 'index.php?fc=module&module=evopayments&controller=response&ipg=0&retry=' . $retry;
            ...
            timestamp(strtotime(date('Y-m-d H:i:s')) * 1000)->
            country(strtoupper($this->context->country->iso_code))->
            currency($this->getCurrencyIsobyId($this->context->cart->id_currency))->
    ...
    public function addNewOrderState($state, $names)
    {
        if (!(Validate::isInt(Configuration::get($state)) && Validate::isLoadedObject($order_state = new OrderState(Configuration::get($state))))) {
            ...
            if (!empty($languages)) {
                foreach ($languages as $step => $lang) {
                    ...
                    if (isset($names[$lang]) && $names[$lang]!=''){
                        $state_isset = $this->checkIfStateExist($id_lang, $names[$lang]);
                        if($state_isset===false) {
                            $order_state->name[$id_lang] = $names[$lang];
                        } else {
                            return $state_isset;
                        }
                    } else {
                        $state_isset = $this->checkIfStateExist($id_lang, $names['default']);
                        if($state_isset===false) {
                            $order_state->name[$id_lang] = $names['default'];
                        } else {
                            return $state_isset;
                        }
                    }
            ...
            $order_state->module_name = 'evopayments';
    ...
    /**
     * get payment status description by EVOPayments status
     */
