...
include_once _PS_MODULE_DIR_ . '/evomexico/translations/statuses.php';

class EVOMexico extends PaymentModule
{
    ...
    const ST_EVO_PAYMENT_URL_LIVE = "https://api.evomexico.mx/payments";
    //DEFAULT TO IFRAME
    // 1 - IFRAME
    // 0 - REDIRECT
    // 2 - HOSTED PAY
    ...
    {
        $this->name = 'evomexico';
        $this->displayName = 'EVOMexico';
        $this->tab = 'payments_gateways';
        $this->version = '1.1.0';
        $this->author = 'EVOMexico';
        ...
        $this->displayName = $this->l('EVOMexico');
        $this->description = $this->l('Accepts payments by EVOMexico');
    ...
    public function displayForm()
    {
        $form['evomexico'] = [
            ...
            $form['merchant'] = [
                'form' => [
                    'legend' => [
                        'title' => $this->l('EVOMexico Connection settings'),
                        'icon' => 'icon-cog'
                    ],
                    'input' => [
                        [
        ...
        $this->addIntegrationMode($form['evomexico']['form']['input']);
    ...
    public function buildTemplatePath($name)
    {
        ...
        return 'module:evomexico/views/templates/front/' . $name . '17.tpl';
    }
    ...
    public function hookPaymentOptions($params)
    {
        ...
        $paymentOption->setCallToActionText($this->l('Pay with EVOMexico'))
...
//         $langCode = (new Language($order->id_lang)) -> iso_code;
        ...
        if (Configuration::get('EVO_REFUND_POSSIBILITY') && $order->module == 'evomexico') {
            $lastStatus = 0;
            ...
            if ($orders) {
                foreach ($orders as &$orderr) {
                    ...
                    if($rawInfo !== ""){
                        ...
                        $orderr['info'] = $this->l('Refunded: ', 'evomexico').$rawInfo;
                    }
    ...
    private function refundProcess($order, $tokenEvo)
    {
        ...
        if($possibleToRefund < 0 || round($refundAmount,2) > round($possibleToRefund,2)){
            return ['state' => 'danger', 'text' => $this->l('You can not make a refund on this amount for this order.', 'evomexico')];
        }
        ...
        if(!$refund->errors){
            ...
            //$order->addOrderPayment('-'.$refundAmount, 'EVOMexico', $id_evo_payment, $currency, date('Y-m-d H:i:s'));
            ...
            return ['state' => 'danger', 'text' => $this->l('No connect with payment gateway.', 'evomexico')];
        }
        return false;
    }
    ...
    private function getSumCurrentOrderPayments($order)
    {
        ...
        foreach ($payments as $payment) {
            if($payment->payment_method=='EVOMexico'){
    ...
    {
        $link = $this->context->link->getModuleLink('evomexico', 'payment');
    ...
    {
        $paymentOptions = [
            'cta_text' => $this->l('Pay with EVOMexico'),
            'action' => $this->context->link->getModuleLink('evomexico', 'payment'),
    ...
    public function getPaymentToken($merchantTxId, $retry='', $mode, $amount='')
    {
        ...
        try {
            ...
            $notificationUrl = Tools::getHttpHost(true).__PS_BASE_URI__ . 'index.php?fc=module&module=evomexico&controller=response&ipg=0&retry=' . $retry;
            $landingUrl = Tools::getHttpHost(true).__PS_BASE_URI__ . 'index.php?fc=module&module=evomexico&controller=response&ipg=0&retry=' 
    ...
    public function addNewOrderState($state, $names)
    {
		...
		$order_state->module_name = 'evomexico';
    ...
    /**
     * get payment status description by EVOMexico status
     */
